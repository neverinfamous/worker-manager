...
export function AddSecretDialog({
    ...
    const handleSubmit = (): void => {
        ...
        if (!validateSecretName(secretName)) {
            setError('Secret name must start with a letter and contain only uppercase letters, numbers, and underscores')
            return
        }
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Add a new secret to <code className="bg-muted px-1 rounded">{workerName}</code>
                    </DialogDescription>
                </DialogHeader>
