...
export function PageCard({ page, onSelect, onSettings }: PageCardProps): React.ReactNode {
    const latestDeploy = page.latest_deployment
    const deployStatus = latestDeploy?.latest_stage?.status ?? 'idle'

    const statusVariant = deployStatus === 'success' ? 'success'
        : deployStatus === 'failure' ? 'destructive'
            : deployStatus === 'active' ? 'warning'
                : 'secondary'
    ...
    return (
        <div
            ...
            onKeyDown={(e) => { if (e.key === 'Enter') onSelect(page) }}
            role="button"
            tabIndex={0}
        >
