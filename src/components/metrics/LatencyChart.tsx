...
// Custom bar shape for individual bar coloring (replaces deprecated Cell component)
// Uses SR-TS-01 pattern for Recharts v3 type compatibility
function BarShape(props: RectangleProps & { index?: number }): React.ReactNode {
  const { index = 0, ...rest } = props;
  return <Rectangle {...rest} fill={colors[index]} />;
}
...
export function LatencyChart({
  ...
  p90,
  p99,
}: LatencyChartProps): React.ReactNode {
  const data = [
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <Tooltip
            ...
            formatter={(value) => [
              ((value as number) ?? 0).toFixed(1) + "ms",
              "Duration",
            ]}
          />
          <Bar
            dataKey="value"
            radius={[4, 4, 0, 0]}
            shape={BarShape as unknown as undefined}
          />
        </BarChart>
      </ResponsiveContainer>
    </div>
