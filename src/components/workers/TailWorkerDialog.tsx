...
export function TailWorkerDialog({
    ...
    const [tailConsumers, setTailConsumers] = useState<TailConsumer[]>(currentTailConsumers)
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent className="max-w-lg">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Connect Workers to receive logs from <code className="bg-muted px-1 rounded">{workerName}</code>
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4 py-4">
                    <div className="space-y-2">
                        ...
                        {tailConsumers.length === 0 ? (
                            <p className="text-sm text-muted-foreground">No tail workers connected</p>
                        ) : (
                            <div className="flex flex-wrap gap-2">
                                {tailConsumers.map((tc) => (
                                    <Badge key={tc.service} variant="secondary" className="gap-1 pr-1">
                                        {tc.service}
                                        {tc.environment && (
                                            <span className="text-xs text-muted-foreground">({tc.environment})</span>
                                        )}
                                        <Button
                                            variant="ghost"
                                            size="icon"
                                            className="h-4 w-4 hover:bg-destructive/20"
                                            onClick={() => { handleRemoveTailWorker(tc.service) }}
                                            disabled={loading}
                                        >
                                            <X className="h-3 w-3" />
                                        </Button>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                            ...
                            >
                                <SelectTrigger className="flex-1">
                                    <SelectValue placeholder={filteredWorkers.length === 0 ? 'No workers available' : 'Select a worker...'} />
                                </SelectTrigger>
                                <SelectContent>
                        ...
                        <p className="text-xs text-muted-foreground">
                            Tail Workers receive console logs and exceptions from the connected worker.{' '}
                            <a
